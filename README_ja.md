...
- **AI チャット（調査モード）** — 質問すると、エージェントがWebで**最新情報**を網羅的に調査し（複数のクエリで検索し、新しい情報源を突き合わせ）、出典付きで詳細に回答します。ノートの検索も行えます。会話はノートとして自動保存され、応答待ちは経過時間をタイムアウトと併記して表示、キャンセルにも対応します。
- **チャットで頼むだけで定期タスク登録** — 「毎朝AIニュースをまとめて」のように依頼すると、Mnemo が定期タスクの依頼だと認識し、内容を実行可能な指示文に書き直してスケジューラに登録します。登録内容は返信で報告されます。**Schedule task** ボタンから手動でも作成できます。
...
- **Markdown 優先ストレージ** — 知識はすべて Markdown として、入れ子（階層）フォルダ（カテゴリ）で整理。バイナリは別ディレクトリに保存。ノートは **YAML（frontmatter）を持たないプレーンな Markdown** です。ファイル名がノートのタイトル、フォルダがカテゴリ、更新日時が新旧の判定に使われるため、Mnemo の外でも同じように扱えます。
...
```
data/
├── notes/         # すべての Markdown。カテゴリごとに分類（<タイトル>.md、YAML無し）
│   ├── inbox/
│   ├── wiki/
...
├── scripts/       # タスクが生成したスクリプト・作業ファイル
└── jobs/          # スケジュールジョブの定義と実行履歴
```

ノートは `notes/<カテゴリ>/<タイトル>.md` として保存され、中身は書いた Markdown だけ
です（先頭の `---` メタデータブロックはありません）。Mnemo 上でノート名を変更すると
ファイル名も変わり、一覧は更新日時順に並ぶため、他のエディタでファイルを編集した場合
も反映されます。旧バージョンで作成したノートには frontmatter が残っていますが、その
まま開けます（ブロックは非表示）。そのノートを保存した時点で取り除かれ、一括での書き
換えは行いません。

`notes/` に置かれるのは Markdown のナレッジのみです。タスクの実行にスクリプトなど
の生成ファイルが必要な場合、エージェントはそれを `scripts/` に書きます（エージェ
ントはデータディレクトリで動作し、`notes/` は検索用に読み取るだけで、作業ファイル
を混在させません）。

旧バージョンからの更新について: 以前エージェントが `data/notes/` 配下に書いた
ファイル（例：`data/notes/scripts/`）はそのまま残ります（削除は行いません）。
ノート一覧から外したい場合は `data/scripts/` へ移動してください。
